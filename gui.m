...
if(isempty(varargin)~=1) % naètení vstupních dat, pokud jsou k dispozici
    PP=varargin{1};
...
varargout{1} = handles.output; % nastavit výstup
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[file,path] = uigetfile('*.wav'); % otevøení souboru
if(isequal(file,0)==false) %pokud se soubor otevøel
    [handles.data,handles.Fs]=audioread([path,file]);
    handles.song = audioplayer(handles.data, handles.Fs); % nastaveni pro pøehrávání
...
% Ovládání tlaèítek pro pøehrávání.
